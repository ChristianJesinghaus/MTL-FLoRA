...
def aggregate_A_matrix(result,layer, weights_dict):
    lora_param = "lora_A"
    A_blocks = []

    for client, A_k in result[layer]["lora_A"].items():
        if not client.startswith("client_"):
            continue
        p_k = weights_dict[client]     # e.g. |D_k| / sum_j |D_j|
    ...
    result[layer][lora_param]["aggregated"] = A_global
    return result

def aggreate_B_matrix(result,layer):
    lora_param = "lora_B"
    B_blocks = []

    for client, B_k in result[layer][lora_param].items():
        if not client.startswith("client_"):
            continue
        B_blocks.append(B_k)
    ...
    if len(B_blocks) == 0:
        ...
        result[layer][lora_param]["aggregated"] = torch.tensor([], device='cpu')
        return result
    ...
    result[layer][lora_param]["aggregated"] = stacked_B
    return result



def aggregate_lambdas(result, layer):
    lora_param = "lora_lambdas"
    
    clients = [k for k in result[layer][lora_param].keys() if k.startswith("client_")]
    num_tasks = len(result[layer][lora_param][clients[0]])  # e.g., Lambda_k is a list/tensor per task
    ...
    for t in range(num_tasks):
        ...
        for client in clients:
            Lambda_k = result[layer][lora_param][client][t]  # task t
    ...
    result[layer][lora_param]["aggregated"] = Lambda_global_tasks
...
def aggregate_B_w_matrix(result, layer, weights_dict):
    """
    ...
    lora_param = "lora_B_w"
    clients = [k for k in result[layer][lora_param].keys() if k.startswith("client_")]
    num_tasks = len(result[layer][lora_param][clients[0]])  # assume same number of tasks
    ...
    result[layer][lora_param]["aggregated"] = aggregated_B_w
...
def convert_result_to_dict(result):
    ...
    for layer in result:
        for lora_param in result[layer]:
            for k in result[layer][lora_param].keys():
    ...
    for layer in result:
        for lora_param in result[layer]:
            ...
            original_name = result[layer][lora_param].get("original_name", None)
            assert original_name is not None, "Original name missing in result structure."
            
            for client_key, value in result[layer][lora_param].items():
    ...
    for layer in result:
        for lora_param in result[layer]:
            agg = result[layer][lora_param].get('aggregated', None)
            if agg is None:
                continue
            # Get original name if available, otherwise construct it
            original_name = result[layer][lora_param].get("original_name", None)
...
def aggregate_lora_parameters(client_weights, weights_dict={ "client_1": 0.6, "client_2": 0.4 }):
    ...
    for item in data:
        ...
        # Store original parameter name
        result[layer][lora_param]["original_name"] = key_string
        
        # Store values for each client
        for client_idx, (key, value) in enumerate(item):
            result[layer][lora_param][f"client_{client_idx + 1}"] = value
    ...
    for layer in result:
        for lora_param in result[layer]:
            if lora_param == "lora_B_w":
                result = aggregate_B_w_matrix(result,layer, weights_dict)

            elif lora_param == "lora_B":
                result = aggreate_B_matrix(result,layer)
            
            elif lora_param == "lora_A":
                result = aggregate_A_matrix(result,layer, weights_dict)
            
            elif lora_param == "lora_lambdas":
                result = aggregate_lambdas(result,layer)

    client_weights = convert_result_to_dict(result)   
